...
@router.get("/{field_mapping_id}", response_model=FieldMappingBase)
...
async def create_field_mapping(
    ...
    # existing_field_mapping = db.query(models.FieldMapping).filter(field_mapping.id == models.FieldMapping.id).first()
...
@router.put("/{field_mapping_id}", response_model=FieldMappingBase)
async def update_field_mapping(
    ...
    field_mapping_update = db.query(models.FieldMapping).filter(field_mapping_id == models.FieldMapping.id).first()
    if not field_mapping_update:
        raise HTTPException(status_code=404, detail="Field mapping with this id not found")

    field_mapping_update.mapping_rules = field_mapping.mapping_rules
    db.commit()
    db.refresh(field_mapping_update)
    return field_mapping_update


@router.delete("/{field_mapping_id}")
